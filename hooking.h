	...
	static void run(ucanvas* canvas) {
		if (render::show_menu) {
			...
			if (ZeroGUI::Window("novoline", &pos, fvector2d{ 500.0f, 900.0f }, render::show_menu, width, height)) {
				...
				{
					ZeroGUI::Text("Organized And Made Better By Ud Man");
				}
			}
		}
	}
...
namespace novoline {
	...
	static void init() {
		...
		if (auto world = uobject::find_object(L"Frontend", reinterpret_cast<uobject*>(-1))) {
			...
			draw_transition_o = vmt<decltype(draw_transition_o)>(std::uintptr_t(viewport), std::uintptr_t(draw_transition), vtables::post_render);
		}
